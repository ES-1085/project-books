...
queer_book_genre <- read_excel("../data/data_genre (1).xlsx")
```
...
```{r}
queer_books_genre <- queer_book_data %>% 
  left_join(queer_book_genre, by = "title")
```


```{r corelation-rating-numrating}
queer_books_genre %>% 
  ggplot(aes( x = num_, y = average_rating)) +
  geom_hex() +
...
```{r check_overlap}
Top_ten_ave <- queer_books_genre %>% 
  select(title, average_rating) %>% 
  arrange(desc(average_rating)) %>% 
  slice(1:10)

bottom_ten_ave <- queer_books_genre %>% 
  select(title, average_rating) %>% 
  arrange((average_rating)) %>% 
  slice(1:10)

Top_ten_num <- queer_books_genre %>% 
...
```{r republished-book}
republished_book <- queer_books_genre %>% 
...
queer_books_genre %>% 
  ggplot(aes(x = fct_rev(fct_infreq(genre_1)), fill = rating_categories)) +
  geom_bar() +
  coord_flip() +
  scale_fill_viridis_d() +
  labs( x = "genre")
