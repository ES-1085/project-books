...
```{r}
queer_books_data %>%
  ggplot(aes(x = fct_rev(fct_infreq(year_published)), fill = binding)) +
  geom_bar() +
  scale_fill_viridis_d() +
  guides(fill = "none") +
  coord_flip() +
...
ggplot(data = Books_and_publishers,
       mapping = aes( y = fct_rev(fct_infreq(publisher)), fill = "average_rating")) +
  geom_bar() +
  scale_fill_viridis_d() +
...
```{r}
queer_books_genre %>% 
  mutate(rating_categories = str_match(average_rating, "0:1")~"1")
```


```{r pivot genre}
book_genre %>% 
  pivot_longer(cols = genre_1:genre_5, names_to = c("genre_1", "genre_2", "genre_3", "genre_4", "genre_5", "genre_6", "genre_7", "genre_8"), names_pattern = "(genre_?)(genre_?)(genre_?)(genre_?)(genre_?)", values_to = "all_genre", values_drop_na = TRUE) %>% 
  select(!genre_1:genre_5)
 # ggplot(x = "genre")+
  #geom_bar()
```
...
```{r}
 book_genre %>% 
  ggplot(aes(x = "average_rating", fill = "genre")) +
  geom_bar()
